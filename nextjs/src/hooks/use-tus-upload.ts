    ...
    try {
      ...
      if (!nextItem) {
        ...
        const allComplete = uploadQueue.length > 0 && 
          uploadQueue.every(item => item.status === 'completed' || item.status === 'error')
      ...
      if (!validation.valid) {
        ...
        processingRef.current = false
        processNextUpload() // Process next file
        return
      }
      ...
      processingRef.current = false
      console.error('Error in processNextUpload:', error)
    }
  }, [uploadQueue, courseId, existingFileNames, topicTags, onUploadComplete, onAllUploadsComplete, onError, sequential, updateQueueItem])
