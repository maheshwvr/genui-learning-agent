...
const getMarkdownComponents = (variant: MarkdownVariant) => {
  ...
  if (variant === 'chat') {
    ...
    return {
      ...
      p: ({ children }: MarkdownComponentProps) => <p className="text-base mb-2 last:mb-0">{children}</p>,
      ...
      ul: ({ children }: MarkdownComponentProps) => <ul className="list-disc pl-6 mb-2 last:mb-0 space-y-1">{children}</ul>,
      ol: ({ children }: MarkdownComponentProps) => <ol className="list-decimal pl-6 mb-2 last:mb-0 space-y-1">{children}</ol>,
      li: ({ children }: MarkdownComponentProps) => <li className="text-base">{children}</li>,
    ...
    return {
      ...baseComponents,
      h1: ({ children }: MarkdownComponentProps) => <h1 className="text-lg font-bold mt-2 mb-1">{children}</h1>,
      h2: ({ children }: MarkdownComponentProps) => <h2 className="text-base font-semibold mt-2 mb-1">{children}</h2>,
      h3: ({ children }: MarkdownComponentProps) => <h3 className="text-base font-medium mt-1 mb-1">{children}</h3>,
      p: ({ children }: MarkdownComponentProps) => <p className="text-base mb-1 last:mb-0">{children}</p>,
      code: ({ children }: MarkdownComponentProps) => <code className="bg-muted px-1 py-0.5 rounded text-sm font-mono">{children}</code>,
      pre: ({ children }: MarkdownComponentProps) => <pre className="bg-muted p-2 rounded text-sm font-mono overflow-x-auto my-1">{children}</pre>,
      ...
      a: ({ children, href }: LinkProps) => <a href={href} className="text-primary hover:underline text-base">{children}</a>,
    };
  }
};
